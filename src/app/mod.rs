...
use crate::{
    model::{
        Timestamp,
        auth::{AuthChallengeContent, AuthInitContent, ClientInfo, SubkeyProof},
    },
...
pub trait AppMethods {
    fn send_auth_init(
        &self,
        auth_init_url: AuthInitUrl,
    ) -> impl std::future::Future<Output = Result<(), crate::router::connector::Error>> + Send;

    fn listen_for_auth_request(
        &self,
    ) -> impl std::future::Future<
        Output = Result<DelayedReply<AuthRequestData>, crate::router::connector::Error>,
    > + Send;
...
impl AppMethods for Connector {
    async fn send_auth_init(
        &self,
        auth_init_url: AuthInitUrl,
    ) -> Result<(), crate::router::connector::Error> {
    ...
    async fn listen_for_auth_request(
        &self,
    ) -> Result<DelayedReply<AuthRequestData>, crate::router::connector::Error> {
        let mut router = self.router().lock().await;
        ...
        let id = router
            ...
            .unwrap();
        let rx = router.subscribe_to_service_request(id.clone()).unwrap();
    ...
    async fn auth_response(
        ...
        router
            .new_service_request::<crate::router::app::AuthChallengeResponse>(
                request.from,
                vec![], // TODO: implement subkeys
                (
                    accept,
                    request.challenge,
                    request.subkey_proof,
                    request.event_id,
                ),
            )
            .unwrap();
...
impl From<(AuthChallengeContent, EventId, nostr::PublicKey)> for AuthRequestData {
    fn from(
        (challenge, event_id, pubkey): (AuthChallengeContent, EventId, nostr::PublicKey),
    ) -> Self {
        Self {
            from: pubkey,
