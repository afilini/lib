...
use nostrstore::{operation::counter::CounterEvent, Database, QueryOptions};
...
impl PortalDB {
    ...
    pub async fn new(keypair: Arc<Keypair>, relays: Vec<String>) -> Result<Arc<Self>, AppError> {
        ...
        let database = PortalDB { database : Arc::new(database) };
        Ok(Arc::new(database))
    }
    ...
    pub async fn increment_counter(&self, key : String) -> Result<(), AppError> {
        self.database.store_event(key, CounterEvent::Increment).await.map_err(|e| {
            AppError::DatabaseError(format!("Failed to increment counter: {}", e))
        })?;
        Ok(())
    }
    pub async fn decrement_counter(&self, key : String) -> Result<(), AppError> {
        self.database.store_event(key, CounterEvent::Decrement).await.map_err(|e| {
            AppError::DatabaseError(format!("Failed to decrement counter: {}", e))
        })?;
        Ok(())
    }

    pub async fn read_counter(&self, key: String) -> Result<i64, AppError> {
        let counter = self.database.read_event::<CounterEvent>(key).await.map_err(|e| {
            AppError::DatabaseError(format!("Failed to get counter: {}", e))
        })?;
        Ok(counter)
    }
